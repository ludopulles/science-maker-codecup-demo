...
function aigetmove(cb) {
	var now = new Date();
	aigetmove_cbs.push(function (res) {
		cb(res - 1, now);
	});
...
function docheckwin() {
	...
	if (win[0]) {
		if (win[0] == (aigoesfirst ? 1 : 2)) {
			alert("The AI has won... Better next time!");
		} else if (win[0] == (aigoesfirst ? 2 : 1)) {
			alert("You have conquered the AI! Good job!")
		} else {
			alert("? win = " + JSON.stringify(win) + ", aigoesfirst = " + aigoesfirst);
		}
